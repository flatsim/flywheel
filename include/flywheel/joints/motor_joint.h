...
private:
    ...
    inline void ApplyImpulse(const Vec2& lambda0, float lambda1);
};

inline MotorJoint::MotorJoint(
    RigidBody* bodyA,
    RigidBody* bodyB,
    const Vec2& anchor,
    float maxJointForce,
...
inline void MotorJoint::Prepare(const Timestep& step)
{
...
inline void MotorJoint::SolveVelocityConstraints(const Timestep& step)
{
...
inline void MotorJoint::ApplyImpulse(const Vec2& lambda0, float lambda1)
{
    // V2 = V2' + M^-1 ⋅ Pc
    // Pc = J^t ⋅ λ
