...
struct SeparationFunction
{
    ...
    inline float FindMinSeparation(float t, int32* idA, int32* idB) const
    {
        Transform tfA, tfB;
    ...
    inline float ComputeSeparation(int32 idA, int32 idB, float t) const
    {
        Transform tfA, tfB;
        motionA.GetTransform(t, &tfA);
        motionB.GetTransform(t, &tfB);
...
static constexpr int32 max_toi_iterations = 20;

inline void ComputeTimeOfImpact(
    const Shape* shapeA, Motion motionA, const Shape* shapeB, Motion motionB, float tMax, TOIOutput* output
)
{
