...
private:
    ...
    inline void ApplyImpulse(const Vec3& lambda);
};

// Revolute joint + Angle joint

inline WeldJoint::WeldJoint(
...
inline void WeldJoint::Prepare(const Timestep& step)
{
...
inline void WeldJoint::SolveVelocityConstraints(const Timestep& step)
{
    MuliNotUsed(step);
...
inline void WeldJoint::ApplyImpulse(const Vec3& lambda)
{
    // V2 = V2' + M^-1 ⋅ Pc
    // Pc = J^t ⋅ λ
