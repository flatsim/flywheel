...
inline Quat::Quat(const Vec3& dir, const Vec3& up)
{
    Mat3 rotation;
...
// Mat3 member function implementations
...
inline Mat3 Mat3::Scale(const Vec2& s)
{
    Mat3 t;
...
inline Mat3 Mat3::Rotate(float z)
{
    float sin = sinf(z);
    float cos = cosf(z);
...
// Mat4 member function implementations
...
inline Mat4 Mat4::Rotate(const Vec3& r)
{
    float sinX = sinf(r.x);
    float cosX = cosf(r.x);
    float sinY = sinf(r.y);
