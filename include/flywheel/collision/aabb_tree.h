...
inline NodeProxy AABBTree::InsertLeaf(NodeIndex leaf)
{
...
inline void AABBTree::RemoveLeaf(NodeIndex leaf)
{
    MuliAssert(0 <= leaf && leaf < nodeCapacity);
    MuliAssert(nodes[leaf].IsLeaf());
...
inline NodeProxy AABBTree::CreateNode(Data* data, const AABB& aabb)
{
...
inline bool AABBTree::MoveNode(NodeIndex node, AABB aabb, const Vec2& displacement, bool forceMove)
{
    MuliAssert(0 <= node && node < nodeCapacity);
...
inline void AABBTree::RemoveNode(NodeIndex node)
{
    MuliAssert(0 <= node && node < nodeCapacity);
    MuliAssert(nodes[node].IsLeaf());
...
inline void AABBTree::Rotate(NodeIndex node)
{
...
inline void AABBTree::Swap(NodeIndex node1, NodeIndex node2)
{
...
inline void AABBTree::Traverse(std::function<void(const Node*)> callback) const
{
    if (root == nullNode)
    {
        return;
...
inline void AABBTree::Query(const Vec2& point, std::function<bool(NodeIndex, Data*)> callback) const
{
    if (root == nullNode)
    {
        return;
...
inline void AABBTree::Query(const AABB& aabb, std::function<bool(NodeIndex, Data*)> callback) const
{
    if (root == nullNode)
    {
        return;
...
inline void AABBTree::AABBCast(
    const AABBCastInput& input, std::function<float(const AABBCastInput& input, Data* data)> callback
) const
{
    const Vec2 p1 = input.from;
    const Vec2 p2 = input.to;
...
inline NodeProxy AABBTree::AllocateNode()
{
...
inline void AABBTree::FreeNode(NodeIndex node)
{
    MuliAssert(0 <= node && node <= nodeCapacity);
