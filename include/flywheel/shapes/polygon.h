...
inline Polygon::Polygon(const Vec2* inVertices, int32 inVertexCount, bool resetPosition, float radius, const Transform& tf)
...
inline Polygon::Polygon(std::initializer_list<Vec2> vertices, bool resetPosition, float radius, const Transform& tf)
    : Polygon(vertices.begin(), int32(vertices.size()), resetPosition, radius, tf)
{
}

inline Polygon::Polygon(float width, float height, float radius, const Transform& tf)
    : Shape(polygon, radius)
{
    vertices = localVertices;
    normals = localNormals;
...
inline Polygon::Polygon(float size, float radius, const Transform& tf)
...
inline Polygon::Polygon(const Polygon& other, const Transform& tf)
    : Polygon(other.vertices, other.vertexCount, false, other.radius, tf)
{
}

inline Shape* Polygon::Clone(Allocator* allocator, const Transform& tf) const
{
...
inline Vec2 Polygon::GetClosestPoint(const Transform& transform, const Vec2& q) const
{
...
inline Edge Polygon::GetFeaturedEdge(const Transform& transform, const Vec2& dir) const
{
...
inline int32 Polygon::GetSupport(const Vec2& localDir) const
{
    int32 index = 0;
...
inline void Polygon::ComputeMass(float density, MassData* outMassData) const
{
...
inline void Polygon::ComputeAABB(const Transform& transform, AABB* outAABB) const
{
...
inline bool Polygon::TestPoint(const Transform& transform, const Vec2& q) const
{
    Vec2 localQ = MulT(transform, q);
...
inline bool Polygon::RayCast(const Transform& transform, const RayCastInput& input, RayCastOutput* output) const
{
