...
inline std::vector<Vec2> ComputeConvexHull(std::span<const Vec2> vertices)
{
    if (vertices.size() < 3)
    {
...
inline Circle ComputeCircle(std::span<const Vec2> points)
{
    std::vector<Vec2> R;
    ...
    return detail::Welzl(copy, R, n);
}

inline std::vector<Polygon> ComputeTriangles(std::span<const Vec2> vertices, std::span<const std::vector<Vec2>> constraints)
{
    std::vector<detail::Tri> triangles = detail::ComputeTriangulation(vertices, constraints);

    std::vector<Polygon> res;
    res.reserve(triangles.size());

    for (const detail::Tri& t : triangles)
    {
...
inline std::vector<Polygon> ComputeDecomposition(std::span<const std::vector<Vec2>> constraints)
{
    std::vector<detail::Tri> triangles = detail::ComputeTriangulation({}, constraints);
    std::vector<detail::Poly> polys(triangles.begin(), triangles.end());

    std::unordered_map<detail::TriEdge, detail::Poly*, detail::TriEdge::Hasher> edge2Poly;
    ...
    repeat:
        ...
        {
            detail::Poly& p = polys[index];

            for (int32 i = 0; i < p.VertexCount(); ++i)
            {
                detail::TriEdge e = p.GetEdge(i);
                ...
                detail::Poly* other = edge2Poly[~e];

                detail::Poly merged = detail::Merge(*other, p, ~e);
                if (!merged.IsConvex())
                {
                    edge2Poly.emplace(e, &p);
                    continue;
...
inline void ComputeConvexHull(const Vec2* vertices, int32 vertexCount, Vec2* outVertices, int32* outVertexCount)
{
    if (vertexCount < 3)
    {
